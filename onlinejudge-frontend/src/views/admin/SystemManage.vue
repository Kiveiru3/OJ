    ...
    <el-card class="main-card card-shadow pro-main-card">
      ...
      <div v-else class="pro-table-shell">
        <el-table v-loading="configLoading" :data="configList" stripe class="pro-table">
          ...
          <el-table-column prop="updateUserId" label="更新人" width="100" align="center" />
    ...
    <el-card class="main-card card-shadow pro-main-card monitor-card">
      ...
      <template v-else>
        <div class="monitor-grid">
          ...
          <div class="monitor-item">
            <div class="monitor-label">提交数</div>
            <div class="monitor-value">{{ monitor.totalSubmissions || 0 }}</div>
            <div class="monitor-sub">
              通过 {{ monitor.acceptedSubmissions || 0 }}（{{ monitor.acceptanceRate ?? 0 }}%）
            </div>
          </div>
          <div class="monitor-item">
            <div class="monitor-label">竞赛（进行中 / 总数）</div>
            <div class="monitor-value">{{ monitor.runningContests || 0 }} / {{ monitor.totalContests || 0 }}</div>
            <div class="monitor-sub">24 小时操作日志：{{ monitor.operationLogs24h || 0 }}</div>
          </div>
          <div class="monitor-item">
            <div class="monitor-label">评测队列</div>
            <div class="monitor-value">{{ monitor.pendingSubmissions || 0 }}</div>
            <div class="monitor-sub">
              <el-tag :type="getQueueTagType(monitor.queueStatus)">
                {{ getQueueStatusText(monitor.queueStatus) }}
              </el-tag>
              <span class="queue-age">最久等待：{{ monitor.oldestPendingMinutes || 0 }} 分钟</span>
            </div>
          </div>
        </div>
        ...
        <div class="runtime-grid">
          <div class="runtime-line">
            <span>运行时长</span>
            <span>{{ formatDuration(monitor.uptimeSeconds || 0) }}</span>
          </div>
          <div class="runtime-line">
            <span>JVM 内存</span>
            <span>{{ monitor.jvmUsedMemoryMb || 0 }}MB / {{ monitor.jvmMaxMemoryMb || 0 }}MB</span>
          </div>
          <el-progress :percentage="memoryUsagePercent" :stroke-width="10" />
          <div class="runtime-line">
            <span>线程数 / CPU 核心数</span>
            <span>{{ monitor.threadCount || 0 }} / {{ monitor.availableProcessors || 0 }}</span>
          </div>
          <div class="runtime-line runtime-time">
            <span>生成时间</span>
            <span>{{ formatDateTime(monitor.generatedAt) }}</span>
          </div>
        </div>
      </template>
    </el-card>
      ...
      <div v-else class="pro-table-shell">
        <el-table v-loading="logLoading" :data="logList" stripe class="pro-table">
          ...
          <el-table-column label="动作" width="150">
            <template #default="{ row }">
              <el-tag size="small" type="info" effect="plain">{{ getActionText(row.action) }}</el-tag>
            </template>
          </el-table-column>
    ...
    <el-card class="main-card card-shadow pro-main-card checklist-card">
      <template #header>
        <div class="card-header pro-card-header">
          <div class="header-left pro-title-group">
            <el-icon class="header-icon pro-title-icon"><DataAnalysis /></el-icon>
            <h2 class="pro-title-text">功能完成度清单</h2>
          </div>
      ...
      <div class="checklist-summary">
        <div class="summary-text">
          已完成 {{ checklistOverview.completedFeatures || 0 }} / {{ checklistOverview.totalFeatures || 0 }}
          ({{ checklistOverview.completionRate ?? 0 }}%)
        </div>
        <el-progress
          :percentage="Math.min(100, Number(checklistOverview.completionRate || 0))"
          :stroke-width="12"
        />
      </div>
      ...
      <div v-else class="pro-table-shell">
        <el-table v-loading="checklistLoading" :data="checklistRows" stripe class="pro-table">
          <el-table-column label="模块" width="140">
            <template #default="{ row }">{{ getModuleText(row.moduleName) }}</template>
          </el-table-column>
          <el-table-column prop="featureName" label="功能" min-width="220" />
          <el-table-column label="状态" width="120" align="center">
            <template #default="{ row }">
              <el-tag :type="row.implemented ? 'success' : 'danger'">
                {{ row.implemented ? '已完成' : '未完成' }}
              </el-tag>
            </template>
          </el-table-column>
          <el-table-column label="覆盖率" width="130" align="center">
            <template #default="{ row }">
              {{ (row.matchedEndpoints || []).length }}/{{ (row.requiredEndpoints || []).length }}
            </template>
          </el-table-column>
          <el-table-column label="要求接口" min-width="520">
            <template #default="{ row }">
              <div class="endpoint-tags">
                <el-tag
                  v-for="endpoint in row.requiredEndpoints || []"
                  :key="endpoint"
                  size="small"
                  :type="isEndpointMatched(row, endpoint) ? 'success' : 'info'"
                >
                  {{ endpoint }}
                </el-tag>
              </div>
          ...
          <template #empty>
            <el-empty description="暂无清单数据" :image-size="80" />
          </template>
        </el-table>
      </div>
    </el-card>
...
const monitor = ref({
  ...
  runningContests: 0,
  operationLogs24h: 0,
  oldestPendingSubmissionTime: null,
...
const isEndpointMatched = (row, endpoint) => {
  return Boolean((row.matchedEndpoints || []).includes(endpoint))
}
...
const logActionOptions = [
  ...
  { label: '退出登录 (LOGOUT)', value: 'LOGOUT' },
...
const getModuleText = (module) => {
  ...
  return MODULE_TEXT_MAP[key] || MODULE_TEXT_MAP[key.toUpperCase()] || key
}

const getActionText = (action) => {
  if (!action) return '-'
  const key = String(action).trim()
  return ACTION_TEXT_MAP[key] || ACTION_TEXT_MAP[key.toUpperCase()] || key
}

const getTargetTypeText = (targetType) => {
  if (!targetType) return '-'
  const key = String(targetType).trim()
  return TARGET_TYPE_TEXT_MAP[key] || TARGET_TYPE_TEXT_MAP[key.toUpperCase()] || key
}
...
onMounted(async () => {
  await Promise.all([loadConfigs(), loadMonitor(), loadLogs(), loadChecklist()])
})
</script>
